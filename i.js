...
/*equation*/var z=seq(fx(p));z[0]=tr(z[0]);if(z.length==1){z=z[0];if(z.ri(2)=="++")V[z.le(-2)]=""+(V[z.le(-2)]*1+1)}return null}function ev(p,t,e){
...
/*fix windows*/var i,w,z;z=V._wi;for(i=0;i<z.length;i++)wicu(z[i])}function fx(y,p){
...
/*handle event*/var z=seq(fx(p));var d=O[z[0]];d.ondragstart=function(){return false};d.onselectstart=function(){return false};d[t]=z[1];if(ne(d[t]))d.addEvent(e,function(f){evr(this,t,f)});else d.addEvent(e,function(f){ev(this,t,f)})}function hc(d){
...
/*parameters*/var i,z;z=y.sp(" ");for(i=0;i<z.l();i++){if(nn($(z[i])))$(z[i]).p(p)}}function plt(p){
/*plot*/var f,he,i,ma,q,sc,sr,wi,y,z;q=seq(p);z=d(q[0]);if(z.id.l()==0)z.p("i,f"+z.ind);sr="";sc="";ss="";y=q[1].smc();wi=100;he=100;ma=0;bg="EEEEEE";for(i=0;i<y.length;i+=2){if(y[i]=="wi")wi=y[i+1];if(y[i]=="he")he=y[i+1];if(y[i]=="ma")ma=y[i+1];if(y[i]=="sr")sr=y[i+1];if(y[i]=="ss")ss=y[i+1];if(y[i]=="bg")bg=y[i+1];if(y[i]=="sc")sc=y[i+1]}f=new Swiff(sr,{width:"100%",height:"100%",container:z,vars:{bg:bg,wi:wi,he:he,ma:ma,sc:sc,ss:ss}});return z}function po(p,e){
...
/*run*/V.RA.push(p);var a,b,d,dd,i,f,o,q,B,e,y;z=z.r("*l","<").r("*m","</").r("*r",">");z=tr(z);while(ne(z)){if(z.l()>2&&z.le(2)=="x-"){i=2;while(i<z.l()&&z.ic(i))i++;z="d[][["+z.s(2,i)+"]]"+z.ri(-i)}a=ob(z,0);b=cb(z,a+1);f=tr(z.s(0,a));q=tr(z.s(a+1,b));if(f!="st")q=fx(q,p);if(f=="p")p.p(q);else if(f=="c"){O[q]=$(q);p.c($(q))}else if(f=="x"){a="";y=q.sc();if(y.l()==1){a=p[y[0]]()}else if(y.l()==2)a=p[y[0]](y[1])}else if(f=="r"){B=q.i("[");e=q.i("=");if(e<0||B>=0&&B<e)r(p,q);else{y=seq(q);o=O[y[0]];r(o,y[1])}}else if(f=="rf"){B=q.i("[");e=q.i("=");if(e<0||B>=0&&B<e)rf(p,q);else{y=seq(q);o=O[y[0]];rf(o,y[1])}}else{if(p.id=="stq"){alert(p.type);d=p[f](q)}else{d=this[f](q)}}z=z.s(b+1,z.l());if(f=="s")z=fx(z);if(d instanceof Array){dd=d;d=d[0]}if(t0(f)){}else if(t1(f)){if(d.id=="file_upload_form"){p.appendChild(d)}else p.c(d)}else{a=ob(z,0);b=cb(z,a+1);r(d,fx(z.s(a+1,b),p));z=z.s(b+1,z.l());if(nn(p.c)&&nn(d)&&(nu(d.add)||(d.add!=0))){p.c(d);if(nn(dd))for(i=1;i<dd.length;i++)p.c(dd[i])};if(f=="wi"){a=ob(z,0);b=cb(z,a+1);r(d.sta,fx(z.s(a+1,b),p));z=z.s(b+1,z.l())}}}V.RA.pop()}function rf(p,z){
...
/*run javascript*/r(db(),fx(z[0]))}function rsz(){
/*resize*/if(v("D"))resize()}function s(p){
/*set variable with fix*/var z=seq(fx(p));V[tr(z[0])]=tr(z[1]);return null}function sh(y){
/*show*/p(y,"[sh]")}function seq(z){
/*split equals*/var a,i;a=[];i=z.i("=");if(i>=0){a.u(fx(z.s(0,i)));a.u(z.s(i+1,z.l()))}else a.u(z);return a}function st(p){
...
/*content size*/s=p.acr.sz();p._cw=mx(p.wcw,s.x);p._ch=mx(p.wch,s.y)}function wicm(p,a,b){
